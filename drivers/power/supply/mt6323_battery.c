...
/*
 * Battery internal resistance vs OCV at 25 degC, from the stock A369i
 * battery profile (r_profile_t2). Ordered from full to empty. R is nearly
 * flat across the usable range; the steep rise only happens below the empty
 * (0%) OCV point.
 */
...
static int mt6323_dod_to_ocv(struct mt6323_battery *bat, int dod)
{
	...
	const struct power_supply_battery_ocv_table *t = info->ocv_table[0];
	int n = info->ocv_table_size[0];
	int cap = (MT6323_DOD_MAX - dod) / 100; /* table capacity is in percent */
	int i;

	if (cap >= t[0].capacity)
		return t[0].ocv;
	if (cap <= t[n - 1].capacity)
		return t[n - 1].ocv;
	...
	/* table is ordered by descending capacity */
	return mt6323_interp(cap, t[i - 1].capacity, t[i - 1].ocv,
			     t[i].capacity, t[i].ocv);
}
...
static int mt6323_read_voltage(struct mt6323_battery *bat, int *volt_uv)
{
	...
	ret = iio_read_channel_processed(bat->vbat_chan, &mv);
	if (ret < 0)
		return ret;
...
static void mt6323_battery_update(struct mt6323_battery *bat)
{
	...
	/* Coulomb-integrate the estimated current into the DOD. */
	ddod = div64_s64((s64)i_ua * MT6323_BAT_POLL_S * MT6323_DOD_MAX,
			 3600LL * bat->qmax_uah);
	bat->dod += ddod;

	/* At rest the terminal voltage is the OCV: re-anchor slowly. */
	...
	bat->status = (i_ua > MT6323_BAT_REST_UA) ?
...
static int mt6323_battery_probe(struct platform_device *pdev)
{
	...
	bat->vbat_chan = devm_iio_channel_get(dev, "voltage");
	if (IS_ERR(bat->vbat_chan))
		return dev_err_probe(dev, PTR_ERR(bat->vbat_chan),
				     "getting voltage channel\n");

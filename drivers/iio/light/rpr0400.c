 ...
 * no readable ID register. The chip can drive a PS-threshold interrupt;
 * this driver currently exposes polled reads only.
 */
...
#define RPR0400_REGULATOR_SETTLE_US	2000
...
static DEFINE_RUNTIME_DEV_PM_OPS(rpr0400_pm_ops,
				 rpr0400_runtime_suspend,
				 rpr0400_runtime_resume,
				 NULL);
...
static int rpr0400_probe(struct i2c_client *client)
{
	...
	fsleep(RPR0400_REGULATOR_SETTLE_US);

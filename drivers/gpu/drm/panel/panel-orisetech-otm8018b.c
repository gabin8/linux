...
static const struct drm_display_mode otm8018b_djn_mode = {
	.clock = (480 + 33 + 6 + 33) * (800 + 28 + 84 + 28) * 60 / 1000,
	.hdisplay = 480,
	.hsync_start = 480 + 33,
	.hsync_end = 480 + 33 + 6,
	.htotal = 480 + 33 + 6 + 33,
...
static void otm8018b_djn_init(struct mipi_dsi_multi_context *ctx)
{
	...
	mipi_dsi_dcs_write_seq_multi(ctx, 0xFF, 0x80, 0x09, 0x01);
	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x80);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xFF, 0x80, 0x09);
	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x80);

	mipi_dsi_dcs_write_seq_multi(ctx, 0xF5, 0x01, 0x18, 0x02, 0x18, 0x10,
					 0x18, 0x02, 0x18, 0x0e, 0x18, 0x0f, 0x20);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x90);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xF5, 0x02, 0x18, 0x08, 0x18, 0x06,
					 0x18, 0x0d, 0x18, 0x0b, 0x18);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0xA0);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xF5, 0x10, 0x18, 0x01, 0x18, 0x14,
					 0x18, 0x14, 0x18);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0xB0);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xF5, 0x14, 0x18, 0x12, 0x18, 0x13,
					 0x18, 0x11, 0x18, 0x13, 0x18, 0x00, 0x00);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x8B);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xB0, 0x40);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0xC0);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xC5, 0x00);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x00);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xD8, 0x43, 0x43);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0xB1);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xC5, 0xA9);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x90);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xC5, 0x96, 0xA7, 0x01);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x82);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xC5, 0xA3);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x81);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xC1, 0x66);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0xA0);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xC1, 0xEA);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0xA1);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xC1, 0x08);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0xA2);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xC0, 0x02, 0x1B);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x80);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xC4, 0x30);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x81);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xC4, 0x83);
	...
	mipi_dsi_dcs_write_seq_multi(ctx, 0xC0, 0x50);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x00);
	mipi_dsi_dcs_write_seq_multi(ctx, 0x36, 0x00);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x90);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xC0, 0x00, 0x44, 0x00, 0x00, 0x00, 0x03);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0xA6);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xC1, 0x01, 0x00, 0x00);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x80);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xCE, 0x87, 0x03, 0x14, 0x86, 0x03, 0x14);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x90);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xCE, 0x33, 0x1E, 0x14, 0x33, 0x1F, 0x14);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0xA0);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xCE, 0x38, 0x03, 0x03, 0x1C, 0x00,
					 0x14, 0x00, 0x38, 0x02, 0x03, 0x1D, 0x00,
					 0x14, 0x00);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0xB0);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xCE, 0x38, 0x01, 0x03, 0x1E, 0x00,
					 0x14, 0x00, 0x38, 0x00, 0x03, 0x1F, 0x00,
					 0x14, 0x00);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0xC0);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xCE, 0x30, 0x00, 0x03, 0x20, 0x00,
					 0x14, 0x00, 0x30, 0x01, 0x03, 0x21, 0x00,
					 0x14, 0x00);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0xD0);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xCE, 0x30, 0x02, 0x03, 0x22, 0x00,
					 0x14, 0x00, 0x30, 0x03, 0x03, 0x23, 0x00,
					 0x14, 0x00);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0xC6);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xCF, 0x01, 0x80);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0xC9);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xCF, 0x10);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0xC0);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xCB, 0x00, 0x54, 0x54, 0x54, 0x54,
					 0x00, 0x00, 0x54, 0x54, 0x54, 0x54, 0x00,
					 0x00, 0x00, 0x00);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0xD0);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xCB, 0x00, 0x00, 0x00, 0x00, 0x00,
					 0x00, 0x54, 0x54, 0x54, 0x54, 0x00, 0x00,
					 0x54, 0x54, 0x54);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0xE0);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xCB, 0x54, 0x00, 0x00, 0x00, 0x00,
					 0x00, 0x00, 0x00, 0x00, 0x00);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x80);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xCC, 0x00, 0x26, 0x25, 0x02, 0x06,
					 0x00, 0x00, 0x0A, 0x0E, 0x0C);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x90);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xCC, 0x10, 0x00, 0x00, 0x00, 0x00,
					 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x26,
					 0x25, 0x01, 0x05);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0xA0);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xCC, 0x00, 0x00, 0x09, 0x0D, 0x0B,
					 0x0F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
					 0x00, 0x00, 0x00);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0xB0);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xCC, 0x00, 0x25, 0x26, 0x05, 0x01,
					 0x00, 0x00, 0x0F, 0x0B, 0x0D);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0xC0);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xCC, 0x09, 0x00, 0x00, 0x00, 0x00,
					 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x25,
					 0x26, 0x06, 0x02);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0xD0);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xCC, 0x00, 0x00, 0x10, 0x0C, 0x0E,
					 0x0A, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
					 0x00, 0x00, 0x00);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x00);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xD9, 0x31);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x00);
	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x00);
	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x00);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xE1, 0x06, 0x07, 0x0E, 0x0D, 0x07,
					 0x16, 0x0C, 0x0C, 0x02, 0x06, 0x05, 0x07,
					 0x0F, 0x2B, 0x27, 0x0D);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x00);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xE2, 0x06, 0x07, 0x0E, 0x0D, 0x07,
					 0x16, 0x0C, 0x0C, 0x02, 0x06, 0x05, 0x07,
					 0x0F, 0x2B, 0x27, 0x0D);

	mipi_dsi_dcs_write_seq_multi(ctx, 0x00, 0x00);
	mipi_dsi_dcs_write_seq_multi(ctx, 0xFF, 0xFF, 0xFF, 0xFF);

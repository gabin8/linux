...
//
// mt6572-afe-pcm.c  --  MediaTek MT6572 AFE platform driver
//
// The mt6572 Audio Front-End: a DL1 playback DAI feeding the ADDA downlink SRC
// and the AFE<->PMIC serial link to the mt6323 codec. The AFE registers sit in
// the parent audsys syscon (phys 0x11140000), mapped with a fast_io regmap so
// the playback trigger and the period IRQ run atomically.
...
static const struct snd_pcm_hardware mt6572_afe_hardware = {
	/* No MMAP: the DL1 buffer is on-chip SRAM (DEV_IRAM). Plain RW keeps
	 * appl_ptr kernel-tracked, so a synced 0 can't clobber it into underrun. */
	...
	.channels_min = 1,
	.channels_max = 2,
	/* Flexible period/buffer so the app's config sets stop_threshold ==
	 * buffer_size; capped at the 16 KiB AFE SRAM. */
			       ...
			       struct snd_pcm_substream *substream)
{
	...
	/*
	 * The datasheet requires AFE_DL1_END[2:0] = 3'h7 (the memif end pointer
	 * is 8-byte granular). Keep the period -- hence the buffer -- a multiple
	 * of 8 bytes so the end address always lands on a full word.
	 */
				    ...
				    struct snd_pcm_hw_params *params)
{
	...
	/* Program the DL1 memif DMA ring (in the AFE on-chip SRAM). End is
	 * inclusive and 8-byte granular (AFE_DL1_END[2:0] = 7); the 8-byte
	 * period constraint in .open keeps the buffer aligned. */
...
/*
 * Process-context setup before the atomic trigger: the AFE path + AFE_ON, then
 * the PMIC analog (stock order: analog after AFE_ON). The trigger then only
 * flips DL1_ON + the period IRQ.
 */
				  ...
				  struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		/*
		 * Minimal stop (mainline AFE FE pattern): disable only the DL1 memif
		 * and its period IRQ; leave the path/analog up (.prepare re-asserts
		 * them). Tearing the whole path down at once clicks; this lets the
		 * ADDA SRC ring the tail down gently. Proper depop is a follow-up.
		 */
				    ...
				    struct snd_soc_pcm_runtime *rtd)
{
	...
	/* DL1 buffer in the AFE on-chip SRAM (16 KiB); no MMAP (RW path). */
...
/*
 * "Playback Volume" = the DL digital gain (DL_SRC2_CON1[31:16]), a linear
 * 16-bit multiplier before the DAC. Custom get/put shadow it in afe->dl_gain
 * so .prepare can re-apply it on every stream.
 */
...
/*
 * AFE MCU interrupt: IRQ1 = a DL1 period boundary. Plain hardirq -- the AFE
 * regmap is fast_io and the PCM is atomic, so snd_pcm_period_elapsed runs here
 * directly (stock/mainline AFE pattern). Active-low.
 */
